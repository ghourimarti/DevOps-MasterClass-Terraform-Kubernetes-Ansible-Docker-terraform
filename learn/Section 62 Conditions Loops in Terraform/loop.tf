# Terraform Function & Loop Examples
locals {
  # -------------------------------------------------
  # base64encode(string)
...
Example output snippets after `terraform apply`:

```bash
base64_decoded = "hello terraform"
...
# Separate Terraform Script: Count Example
...
output "count_result" {
  value = [
    for r in null_resource.server : r.triggers
  ]
}
```
...
# Separate Terraform Script: For Example
...
# Separate Terraform Script: for_each Example
...
output "foreach_result" {
  value = {
    for k, r in null_resource.service :
    k => r.triggers
  }
}
```
